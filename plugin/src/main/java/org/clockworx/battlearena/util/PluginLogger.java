...
import org.clockworx.battlearena.BattleArenaConfig;
...
/**
 * Custom logger for the BattleArena plugin with:
 * - Timestamped output
 ...
 * 
 * Output format: [BattleArena] [LEVEL] [HH:mm:ss] [Category] Message
...
public class PluginLogger {
    ...
    private final BattleArenaConfig config;
     ...
     * @param config The config for debug settings
     */
    public PluginLogger(BattleArena plugin, BattleArenaConfig config) {
    ...
    private void initializeFileLogging() {
        try {
            this.fileLoggingEnabled = isFileLoggingEnabled();
            ...
            String logDirName = getLogDirectory();
            ...
            this.maxFileSizeBytes = getMaxLogFileSizeMb() * 1024L * 1024L;
    ...
    private Path getLogFilePath(LocalDate date, int rotationIndex) {
        String fileName;
        if (rotationIndex == 0) {
            fileName = "battlearena-" + date.format(dateFormatter) + ".log";
        } else {
            fileName = "battlearena-" + date.format(dateFormatter) + "." + rotationIndex + ".log";
        }
    ...
    private void cleanupOldLogs() {
        int maxFiles = getMaxLogFiles();
        if (maxFiles <= 0) {
            return; // Unlimited files
        }
        
        try {
            java.io.File[] logFiles = logDirectory.toFile().listFiles((dir, name) -> 
                name.startsWith("battlearena-") && name.endsWith(".log"));
            ...
            Arrays.sort(logFiles, Comparator.comparingLong(java.io.File::lastModified));
    ...
    private boolean shouldLog(LogCategory category) {
        ...
        // For now, all categories are logged if debug is enabled
        // Can be extended with per-category filtering later
        return true;
    }
    ...
    // ==================== Config Access (delegated to config) ====================
    ...
    private boolean isVerbose() {
        // TODO: Add verbose setting to BattleArenaConfig
        return false;
    }
    
    private boolean shouldIncludeDebugInFile() {
        // TODO: Add include-debug-in-file setting to BattleArenaConfig
        return true;
    }
    
    private boolean isFileLoggingEnabled() {
        // TODO: Add file logging setting to BattleArenaConfig
        return true;
    }
    
    private String getLogDirectory() {
        // TODO: Add log directory setting to BattleArenaConfig
        return "logs";
    }
    
    private int getMaxLogFileSizeMb() {
        // TODO: Add max log file size setting to BattleArenaConfig
        return 10;
    }
    
    private int getMaxLogFiles() {
        // TODO: Add max log files setting to BattleArenaConfig
        return 7;
    }
     ...
     */
    public boolean isDebugEnabledPublic() {
