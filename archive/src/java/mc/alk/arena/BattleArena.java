...
public class BattleArena extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        try {
            ...
            storageManager.initialize().join(); // Wait for initialization
            PlayerStoreController.getPlayerStoreController().setStorageManager(storageManager);
            Log.info("[BattleArena] Storage system initialized");
        } catch (Exception e) {
            Log.err("[BattleArena] Failed to initialize storage system: " + e.getMessage());
            Log.printStackTrace(e);
            // Continue without storage - plugin will work but data won't persist
        }
    ...
    public void onDisable() {
        ...
        if (storageManager != null) {
            try {
                ...
                Log.info("[BattleArena] Storage system shut down");
            } catch (Exception e) {
                Log.err("[BattleArena] Error shutting down storage system: " + e.getMessage());
            }
        }
    }
