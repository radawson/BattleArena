...
import mc.alk.arena.storage.StorageManager;
...
public class BattleArenaExecutor extends CustomCommandExecutor {
    ...
    @MCCommand(cmds = {"migrate"}, admin = true, perm = "battlearena.admin", usage = "migrate <from> <to>", min = 2, max = 2)
    public boolean migrate(CommandSender sender, String[] args) {
        String fromType = args[0].toLowerCase();
        String toType = args[1].toLowerCase();
        
        StorageManager.StorageType from;
        StorageManager.StorageType to;
        
        try {
            from = StorageManager.StorageType.fromId(fromType);
            to = StorageManager.StorageType.fromId(toType);
        } catch (IllegalArgumentException e) {
            return sendMessage(sender, "&cInvalid storage type. Valid types: flatfile, sqlite, mysql");
        }
        
        if (from == to) {
            return sendMessage(sender, "&cSource and destination storage types are the same.");
        }
        
        sendMessage(sender, "&eStarting migration from " + from.name() + " to " + to.name() + "...");
        sendMessage(sender, "&eThis may take a while. Please wait...");
        
        // Run migration asynchronously
        BattleArena.getSelf().getServer().getScheduler().runTaskAsynchronously(BattleArena.getSelf(), () -> {
            try {
                StorageManager storageManager = new StorageManager(BattleArena.getSelf());
                int count = storageManager.migrateData(from, to).join();
                
                BattleArena.getSelf().getServer().getScheduler().runTask(BattleArena.getSelf(), () -> {
                    sendMessage(sender, "&aMigration completed successfully!");
                    sendMessage(sender, "&eMigrated " + count + " player saves.");
                    sendMessage(sender, "&eRemember to update your config.yml to use the new storage type.");
                });
            } catch (Exception e) {
                BattleArena.getSelf().getServer().getScheduler().runTask(BattleArena.getSelf(), () -> {
                    sendMessage(sender, "&cMigration failed: " + e.getMessage());
                    BattleArena.getSelf().getLogger().severe("Migration failed: " + e.getMessage());
                    e.printStackTrace();
                });
            }
        });
