...
public class PlayerStoreController {
    ...
    /**
     * Sets the storage manager for persisting player data.
    ...
    public void storeAll(final ArenaPlayer player) {
        ...
        if (storageManager != null && storageManager.isAvailable()) {
            ...
            if (save != null) {
                storageManager.savePlayerData(player.getID(), save)
                    .exceptionally(ex -> {
                        Log.err("[BattleArena] Failed to persist player data for " + player.getName() + ": " + ex.getMessage());
                        return null;
                    });
            }
        }
    ...
    private PlayerSave getSave(final ArenaPlayer player) {
        ...
        if (save == null && storageManager != null && storageManager.isAvailable()) {
            try {
                save = storageManager.loadPlayerData(player.getID()).join();
                if (save != null) {
                    saves.put(player.getID(), save);
                }
            } catch (Exception e) {
                Log.err("[BattleArena] Failed to load player data from storage for " + player.getName() + ": " + e.getMessage());
            }
        }
    ...
    /**
     * Saves player data to storage when player leaves.
     * Called after restoreAll to persist any remaining data.
     */
    public void saveOnLeave(final ArenaPlayer player) {
        if (storageManager != null && storageManager.isAvailable()) {
            PlayerSave save = getSave(player);
            if (save != null) {
                storageManager.savePlayerData(player.getID(), save)
                    .exceptionally(ex -> {
                        Log.err("[BattleArena] Failed to persist player data on leave for " + player.getName() + ": " + ex.getMessage());
                        return null;
                    });
            }
        }
